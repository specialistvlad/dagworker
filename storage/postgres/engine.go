...
// method, discarded at the bottom, never retained across a call boundary.
type engine struct {
	ctx    context.Context
	tx     pgx.Tx
	scope  string
...
func newEngine(ctx context.Context, tx pgx.Tx, scope string, cfg dw.ScopeConfig, jitter func(int64) int64) *engine {
	return &engine{ctx: ctx, tx: tx, scope: scope, cfg: cfg, jitter: jitter}
}
...
// PhaseDone tie broken by status exactly as memory breaks it.
...
// net-zero write.
func (e *engine) moveBucket(oldPhase dw.Phase, oldStatus dw.Status, newPhase dw.Phase, newStatus dw.Status) error {
	...
	if _, err := e.tx.Exec(e.ctx, sql, e.scope); err != nil {
...
func (e *engine) enterBucket(phase dw.Phase, status dw.Status) error {
...
func (e *engine) leaveBucket(phase dw.Phase, status dw.Status) error {
	col := bucketColumn(phase, status)
	sql := fmt.Sprintf(`UPDATE dagw.scopes SET %s = %s - 1 WHERE scope = $1`, col, col)
	if _, err := e.tx.Exec(e.ctx, sql, e.scope); err != nil {
...
func (e *engine) insertEvent(nodeID, nodeKind string, kind dw.EventKind, from, to dw.Status, reason dw.Reason, message string, attempt uint32, seq dw.Seq) (dw.Effect, error) {
	var cursor int64
	var at time.Time
...
func (e *engine) loadForUpdate(id int64) (nodeRow, error) {
	row := e.tx.QueryRow(e.ctx, `SELECT `+nodeColumns+` FROM dagw.nodes WHERE id = $1 FOR UPDATE`, id)
	return scanNode(row)
}

// loadManyForUpdate locks and returns several nodes, in ascending id order —
// the deterministic order ADR-0038 requires of any write that touches more
// than one row, so that two concurrent cascades whose successor sets overlap
// always request locks in the same order and neither can deadlock the other.
func (e *engine) loadManyForUpdate(ids []int64) ([]nodeRow, error) {
	if len(ids) == 0 {
		return nil, nil
	}
	rows, err := e.tx.Query(e.ctx,
		`SELECT `+nodeColumns+` FROM dagw.nodes WHERE id = ANY($1) ORDER BY id FOR UPDATE`, ids)
	if err != nil {
	...
	for rows.Next() {
		n, err := scanNode(rows)
		if err != nil {
			return nil, fmt.Errorf("postgres: load nodes: scan: %w", err)
		}
		out = append(out, n)
	}
	return out, rows.Err()
...
func (e *engine) successorsForUpdate(fromID int64) ([]nodeRow, error) {
	rows, err := e.tx.Query(e.ctx, `
...
func (e *engine) markEdgeSatisfied(fromID, toID int64) (bool, error) {
	tag, err := e.tx.Exec(e.ctx,
...
func (e *engine) applyDepDelta(succID int64, predStatus dw.Status, predReason dw.Reason) (dw.DepCounts, error) {
	col := "deps_failed"
	switch {
...
RETURNING deps_unsatisfied, deps_succeeded, deps_skipped, deps_failed`, col, col)
	var u, s, sk, f int32
	if err := e.tx.QueryRow(e.ctx, sql, succID).Scan(&u, &s, &sk, &f); err != nil {
...
func (e *engine) makeReady(n *nodeRow) (dw.Effect, error) {
...
RETURNING seq, fifo`, n.ID, int16(dw.PhaseReady), int16(dw.StatusNew)).Scan(&seq, &fifo)
	...
	if err := e.moveBucket(oldPhase, oldStatus, n.Phase, n.Status); err != nil {
		return dw.Effect{}, err
	}
	return e.insertEvent(n.NodeID, n.Kind, dw.EventReady, dw.StatusNew, dw.StatusNew, dw.ReasonNone, "", n.Attempt, n.Seq)
}
...
// neither either).
func (e *engine) makeBlocked(n *nodeRow) error {
	oldPhase, oldStatus := n.Phase, n.Status
	_, err := e.tx.Exec(e.ctx, `UPDATE dagw.nodes SET phase = $2, status = $3 WHERE id = $1`,
	...
	return e.moveBucket(oldPhase, oldStatus, n.Phase, n.Status)
}
...
func (e *engine) schedule(n *nodeRow, delaySeconds float64) error {
	oldPhase, oldStatus := n.Phase, n.Status
...
func (e *engine) settle(n *nodeRow) ([]dw.Effect, error) {
	...
	case n.Deps.Unsatisfiable(n.Trigger):
		...
		return e.terminate(n.ID, dw.StatusError, reason, terminalMessage(reason))
	...
	default:
		if n.Phase != dw.PhaseBlocked {
			if err := e.makeBlocked(n); err != nil {
				return nil, err
			}
		}
	}
...
func (e *engine) terminate(root int64, status dw.Status, reason dw.Reason, message string) ([]dw.Effect, error) {
	type item struct {
		id      int64
		status  dw.Status
		reason  dw.Reason
...
RETURNING seq`, n.ID, int16(dw.PhaseDone), int16(it.status), int16(it.reason), it.message).Scan(&seq)
		...
		ef, err := e.insertEvent(n.NodeID, n.Kind, dw.EventTransition, oldStatus, n.Status, n.Reason, n.Message, n.Attempt, n.Seq)
		if err != nil {
			return nil, err
		}
		effects = append(effects, ef)

		succs, err := e.successorsForUpdate(n.ID)
		if err != nil {
			return nil, err
		}
		for i := range succs {
			succ := &succs[i]
			flipped, err := e.markEdgeSatisfied(n.ID, succ.ID)
			if err != nil {
				return nil, err
			}
			if !flipped {
			...
			deps, err := e.applyDepDelta(succ.ID, n.Status, n.Reason)
			if err != nil {
				return nil, err
			}
			succ.Deps = deps
			...
			case succ.Deps.Unsatisfiable(succ.Trigger):
				...
				queue = append(queue, item{succ.ID, dw.StatusError, rs, terminalMessage(rs)})
			case succ.Deps.Ready(succ.Trigger):
				if succ.Phase != dw.PhaseReady {
					ef, err := e.makeReady(succ)
					if err != nil {
						return nil, err
					}
					effects = append(effects, ef)
...
func (e *engine) notifyIfDirty() error {
	if !e.notify {
		return nil
	}
	if _, err := e.tx.Exec(e.ctx, `SELECT pg_notify($1, $2)`, notifyChannel, e.scope); err != nil {
