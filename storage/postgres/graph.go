...
func (s *Store) AddNodes(ctx context.Context, scope dw.Scope, specs []dw.NodeSpec) ([]dw.Effect, error) {
	...
	eng := newEngine(ctx, tx, string(scope), cfg, s.jitter)

	// Pass one: materialise every spec's node, or verify an existing one is
	// byte-identical. A batch that fails identity here has made no writes yet
	// beyond whichever earlier specs in the same call already created rows —
	// which the deferred Rollback discards along with everything else.
	nodes := make(map[dw.NodeID]nodeRow, len(specs))
	var fresh []dw.NodeID
	for _, spec := range specs {
		existing, err := eng.loadForUpdateByExternal(string(spec.ID))
		switch {
		case err == nil:
		...
		default:
			return nil, fmt.Errorf("postgres: AddNodes: load %q: %w", spec.ID, err)
		}
	}

	// Pass two: link declared dependencies. A dependency may be an earlier
	// spec in this same batch (already in nodes) or must already exist.
	...
	for _, spec := range specs {
		...
		for _, dep := range spec.Deps {
			from, ok := nodes[dep]
			if !ok {
				loaded, err := eng.loadForUpdateByExternal(string(dep))
				if err != nil {
					if isNoRows(err) {
			...
			if err := eng.linkDependency(&from, &to); err != nil {
				return nil, err
			}
			nodes[dep] = from
			nodes[spec.ID] = to
	...
	// Pass three: announce creation, then settle every touched node against a
	// freshly re-read row — never the pass-two snapshot, so there is no
	// question of a cascade from one touched node's settle having already
	// moved another touched node before its own turn arrives.
	var effects []dw.Effect
	for _, extID := range fresh {
		n := nodes[extID]
		seq, err := eng.bumpSeq(n.ID)
		if err != nil {
			return nil, err
		}
		n.Seq = seq
		ef, err := eng.insertEvent(n.NodeID, n.Kind, dw.EventCreated, dw.StatusNew, n.Status, n.Reason, n.Message, n.Attempt, n.Seq)
		if err != nil {
			return nil, err
		}
		effects = append(effects, ef)
	}
	for _, id := range touched.items {
		fresh, err := eng.loadForUpdate(id)
		if err != nil {
			return nil, fmt.Errorf("postgres: AddNodes: settle: load: %w", err)
		}
		more, err := eng.settle(&fresh)
		if err != nil {
			return nil, err
		}
...
func (s *Store) AddEdges(ctx context.Context, scope dw.Scope, edges []dw.Edge) ([]dw.Effect, error) {
	...
	eng := newEngine(ctx, tx, string(scope), dw.ScopeConfig{}, s.jitter)
	...
	load := func(id dw.NodeID) (nodeRow, error) {
		...
		n, err := eng.loadForUpdateByExternal(string(id))
		if err != nil {
			return nodeRow{}, err
		}
		nodes[id] = n
	...
	for _, id := range touched.items {
		fresh, err := eng.loadForUpdate(id)
		if err != nil {
			return nil, fmt.Errorf("postgres: AddEdges: settle: load: %w", err)
		}
		more, err := eng.settle(&fresh)
		if err != nil {
			return nil, err
...
func (s *Store) RemoveEdges(ctx context.Context, scope dw.Scope, edges []dw.Edge) ([]dw.Effect, error) {
	...
	for _, e := range edges {
		...
		changed, err := eng.unlinkDependency(fromID, toID)
		if err != nil {
			return nil, err
		}
		if changed {
	...
	for _, id := range touched.items {
		fresh, err := eng.loadForUpdate(id)
		if err != nil {
			return nil, fmt.Errorf("postgres: RemoveEdges: settle: load: %w", err)
		}
		more, err := eng.settle(&fresh)
		if err != nil {
			return nil, err
...
func (s *Store) RemoveNode(ctx context.Context, scope dw.Scope, id dw.NodeID, policy dw.CascadePolicy) ([]dw.Effect, error) {
	...
	succs, err := eng.successorsForUpdate(n.ID)
	if err != nil {
	...
	if len(succs) > 0 {
		...
		case dw.CascadeFail:
			...
			// terminate re-reads each one, so the locks successorsForUpdate
			// just took are released as soon as this reload takes its own.
			for _, w := range succs {
				more, err := eng.terminate(w.ID, dw.StatusError, dw.ReasonRemoved, terminalMessage(dw.ReasonRemoved))
				if err != nil {
					return nil, err
				}
				effects = append(effects, more...)
		...
		case dw.CascadeDetach:
			// Dropping the edges below is the whole policy; each successor is
			// settled afterward.
		}
	}
	...
	for i, w := range succs {
		successorIDs[i] = w.ID
		if _, err := eng.unlinkDependency(n.ID, w.ID); err != nil {
	...
	if err := eng.leaveBucket(n.Phase, n.Status); err != nil {
		return nil, err
	}
	...
	if policy == dw.CascadeDetach {
		for _, wid := range successorIDs {
			fresh, err := eng.loadForUpdate(wid)
			if err != nil {
				return nil, fmt.Errorf("postgres: RemoveNode: settle successor: %w", err)
			}
			more, err := eng.settle(&fresh)
			if err != nil {
				return nil, err
...
func (s *Store) Cancel(ctx context.Context, scope dw.Scope, ids []dw.NodeID) ([]dw.Effect, error) {
	...
	for _, id := range ids {
		...
		more, err := eng.terminate(internalID, dw.StatusError, dw.ReasonCancelled, terminalMessage(dw.ReasonCancelled))
		if err != nil {
			return nil, err
		}
		effects = append(effects, more...)
...
func (s *Store) CancelScope(ctx context.Context, scope dw.Scope) ([]dw.Effect, error) {
	...
	var effects []dw.Effect
	for _, id := range ids {
		more, err := eng.terminate(id, dw.StatusError, dw.ReasonCancelled, terminalMessage(dw.ReasonCancelled))
		if err != nil {
			return nil, err
		}
		effects = append(effects, more...)
